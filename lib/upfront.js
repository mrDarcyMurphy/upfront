...
upfront.setup = function(opts, done) {
  ...
  if (!opts.app)                { throw new Error("app doesn't exist."); }
  if (!opts.app.settings)       { throw new Error("app.settings doesn't exist."); }
  if (!opts.app.settings.views) { throw new Error("Views path doesn't exist.");  }
