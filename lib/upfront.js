...
upfront.setup = function(opts, done) {
  ...
  if (typeof done !== 'function') { throw new Error('Callback is not a function. If you want to add a config change app to an object.')}
      ...
      fs.stat(file, function(err, stat) {
        if (stat && stat.isDirectory()) {
          ...
          if ( file.match(upfront.config.rx) ){
            ...
            if ( !_.include(upfront.config.ignore, slug) ) {
